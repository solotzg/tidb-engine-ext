    ...
    fn check_for_newer_version<S: Snapshot>(
        ...
        match reader.seek_write(&self.key, TimeStamp::max())? {
            Some((commit_ts, write)) => {
                ...
                // start timestamp, we should abort current prewrite.
...
pub mod tests {
    ...
    fn test_resend_prewrite_non_pessimistic_lock() {
        ...
        // This is a re-sent prewrite. It should report a WriteConflict. In production,
        // the caller will need to check if the current transaction is already
        // committed before, in order to provide the idempotency.
        ...
        // checks. It reports a WriteConflict.
        let err = must_retry_pessimistic_prewrite_put_err(
            &engine,
            b"k2",
            b"v2",

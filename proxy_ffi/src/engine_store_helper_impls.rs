...
impl EngineStoreServerHelper {
    ...
    pub fn wb_put_page(&self, wb: RawVoidPtr, page_id: BaseBuffView, page: BaseBuffView) {
        debug_assert!(self.fn_wb_put_page.is_some());
        unsafe { (self.fn_wb_put_page.into_inner())(wb, page_id, page) }
    }

    pub fn wb_del_page(&self, wb: RawVoidPtr, page_id: BaseBuffView) {
        debug_assert!(self.fn_wb_del_page.is_some());
        unsafe { (self.fn_wb_del_page.into_inner())(wb, page_id) }
    }

    pub fn get_wb_size(&self, wb: RawVoidPtr) -> u64 {
        debug_assert!(self.fn_get_wb_size.is_some());
        unsafe { (self.fn_get_wb_size.into_inner())(wb) }
    }

    pub fn is_wb_empty(&self, wb: RawVoidPtr) -> u8 {
        debug_assert!(self.fn_is_wb_empty.is_some());
        unsafe { (self.fn_is_wb_empty.into_inner())(wb) }
    }

    pub fn merge_wb(&self, lwb: RawVoidPtr, rwb: RawVoidPtr) {
        debug_assert!(self.fn_handle_merge_wb.is_some());
        unsafe { (self.fn_handle_merge_wb.into_inner())(lwb, rwb) }
    }

    pub fn clear_wb(&self, wb: RawVoidPtr) {
        debug_assert!(self.fn_handle_clear_wb.is_some());
        unsafe { (self.fn_handle_clear_wb.into_inner())(wb) }
    }

    pub fn consume_wb(&self, wb: RawVoidPtr) {
        debug_assert!(self.fn_handle_consume_wb.is_some());
        unsafe { (self.fn_handle_consume_wb.into_inner())(self.inner, wb) }
    }
    ...
    pub fn get_lower_bound(&self, page_id: BaseBuffView) -> CppStrWithView {
        debug_assert!(self.fn_handle_get_lower_bound.is_some());
        unsafe { (self.fn_handle_get_lower_bound.into_inner())(self.inner, page_id) }
    }
    ...
    pub fn purge_ps(&self) {
        debug_assert!(self.fn_handle_purge_ps.is_some());
        unsafe { (self.fn_handle_purge_ps.into_inner())(self.inner) }
    }

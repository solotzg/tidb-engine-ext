...
// ReadableDuration::millis(10);     cluster.cfg.raft_store.
// raft_election_timeout_ticks = 10;     cluster.cfg.raft_store.
// raft_store_max_leader_lease = ReadableDuration::millis(90);     cluster.
// pd_client.disable_default_operator();
//
//     cluster.run();
//     cluster.must_put(b"k1", b"v1");
...
//     );
//     let req = new_admin_request(region.get_id(), region.get_region_epoch(),
// cmd);     // Though it has a force leader now, but the command can't
// committed because the log is not replicated to all the alive peers.
//     assert!(
//         cluster
//             .call_command_on_leader(req, Duration::from_millis(1000))
//             .unwrap()
//             .get_header()
//             .has_error() // error "there is a pending conf change" indicating
// no committed log after being the leader     );
//
//     // Permit snapshot message, snapshot should be applied and advance commit
// index now.     cluster.sim.wl().clear_recv_filters(2);
//     cluster
//         .pd_client
//         .must_remove_peer(region.get_id(), find_peer(&region,
// 3).unwrap().clone());     cluster
//         .pd_client
//         .must_remove_peer(region.get_id(), find_peer(&region,
// 4).unwrap().clone());     cluster
//         .pd_client
//         .must_remove_peer(region.get_id(), find_peer(&region,
// 5).unwrap().clone());     cluster.exit_force_leader(region.get_id(), 1);
//
//     // quorum is formed, can propose command successfully now
//     cluster.must_put(b"k4", b"v4");
//     assert_eq!(cluster.must_get(b"k2"), None);
//     assert_eq!(cluster.must_get(b"k3"), None);
//     assert_eq!(cluster.must_get(b"k4"), Some(b"v4".to_vec()));
//     cluster.must_transfer_leader(region.get_id(), find_peer(&region,
// 1).unwrap().clone()); }

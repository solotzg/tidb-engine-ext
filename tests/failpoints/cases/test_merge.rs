...
fn test_merge_election_and_restart() {
    ...
    // Now region 1 can still be merged into region 2 because leader has committed
    // index cache.
    ...
    // If logs from different term are process correctly, store 2 should have latest
    // updates.
...
// peers Disable for strange error:
// thread 'cases::test_merge::test_destroy_source_peer_while_merging' panicked
// at '1 failed to try merge to 1000, resp header { error { message:
// "\"[components/raftstore/src/store/peer.rs:3972]: log gap from matched: 0 or
// committed: 0 to last index: 10 is too large, skip merge\"" } current_term: 7
// }', /home/runner/work/tidb-engine-ext/tidb-engine-ext/components/
// test_raftstore/src/cluster.rs:1686:13 #[test]

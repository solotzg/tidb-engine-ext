...
if [[ $M == "fmt" ]]; then
    ...
    cargo clippy --features "$ENABLE_FEATURES" --package engine_store_ffi --no-deps -- -Dwarnings -A clippy::needless_borrow -A clippy::clone_on_copy -A clippy::upper_case_acronyms -A clippy::missing_safety_doc
    cargo clippy --features "$ENABLE_FEATURES" --package proxy_tests  --no-deps -- -Dwarnings -A clippy::needless_borrow -A clippy::clone_on_copy -A clippy::upper_case_acronyms -A clippy::missing_safety_doc
    cargo clippy --features "$ENABLE_FEATURES" --package proxy_server  --no-deps -- -Dwarnings -A clippy::needless_borrow -A clippy::clone_on_copy -A clippy::upper_case_acronyms -A clippy::missing_safety_doc -A clippy::derive_partial_eq_without_eq
    cargo clippy --features "$ENABLE_FEATURES" --package new-mock-engine-store  --no-deps -- -Dwarnings -A clippy::needless_borrow -A clippy::clone_on_copy -A clippy::upper_case_acronyms -A clippy::missing_safety_doc -A clippy::derive_partial_eq_without_eq -A clippy::redundant_clone -A clippy::too_many_arguments

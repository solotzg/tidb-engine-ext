...
impl PageStorageExt {
    ...
    pub fn write_batch_put_page(&self, wb: RawVoidPtr, page_id: &[u8], page: &[u8]) {
        self.helper().wb_put_page(wb, super::key_format::add_kv_engine_prefix(page_id).into(), page.into())
    }

    pub fn write_batch_del_page(&self, wb: RawVoidPtr, page_id: &[u8]) {
        self.helper().wb_del_page(wb, super::key_format::add_kv_engine_prefix(page_id).into())
    }

    pub fn read_page(&self, page_id: &[u8]) -> Option<Vec<u8>> {
        // TODO maybe we can steal memory from C++ here to reduce redundant copy?
        let value = self.helper().read_page(super::key_format::add_kv_engine_prefix(page_id).into());
    ...
    pub fn scan_page(
        ...
        let values = self
            .helper()
            .scan_page(super::key_format::add_kv_engine_prefix(start_page_id).into(), super::key_format::add_kv_engine_prefix(end_page_id).into());

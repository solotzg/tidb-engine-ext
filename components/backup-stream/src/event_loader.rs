...
impl<S: Snapshot> EventLoader<S> {
    ...
    /// scan a batch of events from the snapshot. Tracking the locks at the same
    /// time. note: maybe make something like [`EntryBatch`] for reducing
    /// allocation.
    fn scan_batch(
        &mut self,
        batch_size: usize,
    ...
    RT: RaftStoreRouter<E>,
{
    ...
    fn observe_over(&self, region: &Region, cmd: ChangeObserver) -> Result<impl Snapshot> {
        ...
        // `SignificantMsg::CaptureChange` to      register the region to CDC
        // observer and get a snapshot at the same time. Registering the
        // observer to the raftstore is necessary because we should only listen events
        // from leader. In CDC, the change observer is per-delegate(i.e.
        // per-region), we can create the command per-region here too.
    ...
    pub fn initialize_range(&self, start_key: Vec<u8>, end_key: Vec<u8>) -> Result<()> {
        ...
        loop {
            ...
            for r in regions {
                ...
                //       For example, if a region cannot elect the leader for long time. (say,
                // net work partition)       At that time, we have nowhere to
                // record the lock status of this region.

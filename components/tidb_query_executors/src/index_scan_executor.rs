...
impl<S: Storage> BatchIndexScanExecutor<S> {
    pub fn new(
        ...
        // transaction buffer or with a SelectLock/pessimistic lock, we
        // need to return the physical table id since several partitions may be included
        // in the range.
        //
...
impl ScanExecutorImpl for IndexScanExecutorImpl {
    ...
    // 		|     In v5.0, restored data contains only non-binary data(except for char
    // and _bin). In the above example, the restored data contains only the value of
    // b. 		|     Besides, if the collation of b is _bin, then restored data is an
    // integer indicate the spaces are truncated. Then we use sortKey 		|     and
    // the restored data together to restore original data.
    #[inline]
    fn process_kv_pair(
        &mut self,
        mut key: &[u8],
...
impl IndexScanExecutorImpl {
    ...
    // column's collation is _bin, then we only write the number of the truncated
    // spaces to value. 3. If a string column is char, not varchar, then we use
    // the sortKey directly. The whole logic of this function is:

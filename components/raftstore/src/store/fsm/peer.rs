    ...
    ER: RaftEngine,
{
    ...
    fn on_raft_base_tick(&mut self) {
        ...
        if self.ctx.cfg.hibernate_regions {
            if self.fsm.hibernate_state.group_state() == GroupState::Idle {
                ...
                // the leader can wake up and broadcast heartbeats in one `raft_heartbeat_ticks`
    ...
    fn on_raft_message(&mut self, msg: InspectedRaftMessage) -> Result<()> {
        ...
        if is_snapshot {
            if !self.fsm.peer.has_pending_snapshot() {
                ...
                // latest snapshot region after applying that snapshot.
    ...
    fn check_snapshot(&mut self, msg: &RaftMessage) -> Result<Either<SnapKey, Vec<(u64, bool)>>> {
        ...
        if self.fsm.peer.local_first_replicate && !self.fsm.peer.is_initialized() {
            ...
            // one. So there should be no initialized peer before.
            // 2. If the peer is also created by splitting, then the snapshot range is not
            // overlapped with    parent peer. It means leader has applied merge
            // and split at least one time. However,    the prerequisite of
            // merge includes the initialization of all target peers and source peers,
    ...
    fn on_check_peer_stale_state_tick(&mut self) {
        ...
        if let Some(ForceLeaderState::ForceLeader { time, .. }) = self.fsm.peer.force_leader {
            // If the force leader state lasts a long time, it probably means PD recovery
            // process aborts for some reasons. So just exit to avoid blocking
            // the read and write requests for this peer.

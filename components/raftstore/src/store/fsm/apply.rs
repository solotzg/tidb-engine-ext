    ...
    EK: KvEngine,
{
    ...
    fn handle_raft_committed_entries(
        ...
        while let Some(entry) = committed_entries_drainer.next() {
            ...
            // `unimplemented!()`, which can break compatibility (i.e. old
            // version tikv running on data written by new version tikv), but PD
            // will reject old version tikv join the cluster, so this should not happen.
    ...
    fn apply_raft_cmd(
        ...
        let (resp, exec_result) = if ctx.host.pre_exec(&self.region, req, index, term) {
            ...
            // We do not need to hack here. A filtered command can also be handled by
            // post_exec.
    ...
    EK: KvEngine,
{
    ...
    // 5.   `on_ready_prepare_merge` in source peer fsm and set pending_remove
    // (means source region has finished applying all logs)
    // 6.   `logs_up_to_date_for_merge` in source apply fsm (destroy its apply fsm
    // and send Noop to trigger the target apply fsm) 7.   resume
    // `exec_commit_merge` in target apply fsm 8.   `on_ready_commit_merge` in
    // target peer fsm and send `MergeResult` to source peer fsm

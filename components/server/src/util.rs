...
use raftstore::engine_store_ffi::{ProtoMsgBaseBuff, get_engine_store_server_helper};
use kvproto::diagnosticspb::{
    ServerInfoRequest, ServerInfoResponse, ServerInfoType,
};
...
pub extern "C" fn ffi_server_info(
    proxy_ptr: RaftStoreProxyPtr, view: BaseBuffView, res: RawVoidPtr) -> u32 {
    ...
    unsafe {
        ...
        get_engine_store_server_helper().set_server_info_resp(
            r.borrow().into(),
            res
        );
    }
